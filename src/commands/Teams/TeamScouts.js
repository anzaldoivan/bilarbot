...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("buscar")
    .setDescription(
      "Buscar jugadores en el Boletin de Jugadores Buscando Equipo"
    )
  ...
  async execute(interaction) {
    ...
    embed = new Discord.MessageEmbed()
      .setTitle(`Jugadores buscando Equipo (${pos})`)
      .setColor("#B9F2FF");

    Object.keys(scouts).forEach(function (key) {
      //console.log(scouts[key].pos);
      //console.log(position);
      if (scouts[key].pos == position) {
        //console.log("Posicion encontrada");

        embed.addField(
          `Jugador`,
          `${scouts[key].ping}\n${scouts[key].descripcion}\nExperiencia: ${scouts[key].experiencia} horas`
        );
      }
    });
